...
const MAP = {
  ...
  solutions: {
    ...
    aFila: s => ({id:s.id, nombre:s.nombre, modo:s.modo||'receta',
                  salt_id: s.modo==='potencia' ? null : s.saltId,
                  gramos:  s.modo==='potencia' ? null : +s.gramos,
                  tipo:       s.modo==='potencia' ? s.tipo : null,
                  ref_delta:  s.modo==='potencia' ? +s.refDelta  : null,
                  ref_ml:     s.modo==='potencia' ? +s.refMl     : null,
                  ref_litros: s.modo==='potencia' ? +s.refLitros : null,
                  volumen_ml:+s.volumenMl, fecha:s.fecha, restante_ml:+s.restanteMl||0,
                  horneado:!!s.horneado, nota:s.nota||'', receta:s.receta||''}),
    aApp : r => ({id:r.id, nombre:r.nombre, modo:r.modo||'receta',
                  saltId:r.salt_id, gramos:r.gramos==null?null:+r.gramos,
                  tipo:r.tipo,
                  refDelta:  r.ref_delta==null?null:+r.ref_delta,
                  refMl:     r.ref_ml==null?null:+r.ref_ml,
                  refLitros: r.ref_litros==null?null:+r.ref_litros,
                  volumenMl:+r.volumen_ml, fecha:r.fecha, restanteMl:+r.restante_ml,
                  horneado:!!r.horneado, nota:r.nota||'', receta:r.receta||''}),
  },
  doses: {
